...
_timer = new Timer(_ => { _running = false; job.State = ClientState.Completed; }, null, job.Duration * 1000, job.Duration * 1000);
            for (var i = 0; i < _job.Connections; ++i)
            {
                _tasks.Add(Task.Run(async () =>
                {
                    while (_running)
                    {
                        await _httpClient.SendAsync(new HttpRequestMessage(HttpMethod.Get, job.ServerBenchmarkUri));
